...
class ReadSettings{
                    ...
                    {                     
                        ...
                        {                                               
                            ...
                            if( rule.is_object() && rule.as_object().contains("conditions") )
                            {
                                ...
                                {
                                    ConditionContext conContext = { context, condition.value().as_string().c_str()};                           
                                    plist->push_back(make_shared<SoConditionCmd>( "./plugins/libCon_"+string(condition.key()) +"_Handler.so", conContext));
                                }                           
                            }
    ...
    static void make_chain_of_responsibility( json::value j )
    {
        ...
        for( auto plugin : plugin_pathes )
        {
            ...
            IoC::Resolve<ICommandPtr, string, IResolverContainerPtr>("IoC.Register", "Http.Res." +to_string(plugin.at("code").as_int64()) , pDefaultCmdContainer)->Execute(); 
        }
                ...
                make_container (  
                    ...
                    {                     
                        ...
                        for( int i = plugin_pathes.size()-1; i >= 0; --i )
                        {
                            ...
                            ICommandPtr cmd = IoC::Resolve<ICommandPtr>("Http.Res."+ to_string(path.at("code").as_int64()), path2so, context );                         
